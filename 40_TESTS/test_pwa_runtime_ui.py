...
async def run() -> None:
    ...
    with local_server() as base:
        async with async_playwright() as pw:
            ...
            check("사람의 일반망 복귀 확인 뒤에만 기존 폐기 준비", await page.get_attribute("#replacementGuard", "data-replacement-state") == "ready_to_replace" and not await existing_row.get_by_text("기존 설정 폐기", exact=True).is_disabled())
            await page.evaluate("go('devices')")
            await page.evaluate("window.confirm=()=>false")
            await existing_row.get_by_text("기존 설정 폐기", exact=True).click()
